...
class CommentModel{
    ...
    function get($id)
    {
        $sentencia = $this->db->prepare('SELECT comentario.id, comentario.comentario, comentario.puntaje, auto.id_auto, usuario.username FROM comentario JOIN automotor JOIN usuario ON comentario.id_usuario = usuario.id AND comentario.id_auto= automotor.id_auto WHERE automotor.id_auto = ?');
